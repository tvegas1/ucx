...
UCS_TEST_P(test_srd, fence_orders)
{
    mapped_buffer srcbuf(16, 0ul, *m_e2);
    mapped_buffer dstbuf(16, 0ul, *m_e1);
    int reordered = 0;

    auto src = reinterpret_cast<volatile uint64_t*>(srcbuf.ptr());
    auto dst = reinterpret_cast<volatile uint64_t*>(dstbuf.ptr());
    ...
    for (auto count = 1000; count > 0; count--) {
        ...
        m_comp.m_count = 0;
        {
            UCS_TEST_GET_BUFFER_IOV(iov, iovcnt, &dst[0], sizeof(dst[0]),
                                    dstbuf.memh(), 1);
            ucs_status_t status = uct_ep_get_zcopy(m_e1->ep(0), iov, iovcnt,
        ...
        {
            UCS_TEST_GET_BUFFER_IOV(iov, iovcnt, &dst[1], sizeof(dst[1]),
                                    dstbuf.memh(), 1);
            ucs_status_t status = uct_ep_get_zcopy(m_e1->ep(0), iov, iovcnt,
        ...
        while (((volatile uint64_t *)dst)[1] != 0x1234)
            ; // poll

        reordered += (src[0] != 0xabcd);

        wait_for_value(&m_comp.m_count, 1, true);
        ASSERT_EQ(1, m_comp.m_count);
    }
