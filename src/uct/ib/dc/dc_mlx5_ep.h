...
#define UCT_DC_MLX5_IFACE_TXQP_GET(_iface, _ep, _index, _txqp, _txwq) \
    UCT_DC_MLX5_IFACE_TXQP_DCI_GET(_iface, (_ep)->dci[_index].id, _txqp, _txwq)
...
enum {
    UCT_DC_MLX5_EP_DCI           = 0,
    UCT_DC_MLX5_EP_DCI_RDMA_READ = 1,
    UCT_DC_MLX5_EP_DCI_COUNT
};
...
struct uct_dc_mlx5_ep {
    ...
    struct uct_dc_mlx5_ep_dci {
        ...
        uint8_t             id;
        uint16_t            flags;
        struct uct_dc_mlx5_ep *ep;
        int                 index;
    } dci[UCT_DC_MLX5_EP_DCI_COUNT];
...
{
    return ep->dci[UCT_DC_MLX5_EP_DCI].flags & UCT_DC_MLX5_EP_FLAG_POOL_INDEX_MASK;
}
                              ...
                              int index)
{
    ...
    return &iface->tx.dcis[ep->dci[index].id].arb_group;
}
                         ...
                         int index)
{
    return (uct_dc_mlx5_iface_is_dci_shared(iface)) ?
            uct_dc_mlx5_ep_rand_arb_group(iface, ep, index) :
            &ep->dci[index].arb_group;
}
    ...
    if (uct_dc_mlx5_iface_is_dci_shared(iface)) {
        ucs_arbiter_group_schedule(uct_dc_mlx5_iface_tx_waitq(iface),
                                   uct_dc_mlx5_ep_rand_arb_group(iface, ep,
                                                                 index));
    } else if (uct_dc_mlx5_iface_dci_has_tx_resources(iface, ep->dci[index].id)) {
        ucs_arbiter_group_schedule(uct_dc_mlx5_iface_tx_waitq(iface),
                                   &ep->dci[index].arb_group);
    }
}
...
{
    ucs_arbiter_group_init(&ep->dci[UCT_DC_MLX5_EP_DCI].arb_group);
    ucs_arbiter_group_init(&ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].arb_group);

    if (uct_dc_mlx5_iface_is_dci_rand(iface)) {
        /* coverity[dont_call] */
        ep->dci[UCT_DC_MLX5_EP_DCI].id =
            rand_r(&iface->tx.rand_seed) % iface->tx.ndci;

        /* Need a different DCI for RDMA reads that will be split */
        do {
            ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].id =
                rand_r(&iface->tx.rand_seed) % iface->tx.ndci;
        } while (ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].id
                 == ep->dci[UCT_DC_MLX5_EP_DCI].id);

        ep->dci_channel_index = 0;
    } else if (uct_dc_mlx5_iface_is_hw_dcs(iface)) {
        ucs_assertv(iface->tx.ndci == 1, "ndci=%u", iface->tx.ndci);
        ep->dci[UCT_DC_MLX5_EP_DCI].id           = 0;
        ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].id = 0;
        ep->dci_channel_index = iface->tx.dcis[0].next_channel_index++ %
                                iface->tx.num_dci_channels;
    } else {
        ep->dci[UCT_DC_MLX5_EP_DCI].id           = UCT_DC_MLX5_EP_NO_DCI;
        ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].id = UCT_DC_MLX5_EP_NO_DCI;
        ep->dci_channel_index = 0;
    }
                                                    ...
                                                    int index)
{
    ...
    return (!(ep->dci[index].flags & UCT_DC_MLX5_EP_FLAG_TX_WAIT)) &&
           uct_rc_fc_has_resources(&iface->super.super, &ep->fc) &&
           uct_dc_mlx5_iface_dci_has_tx_resources(iface, ep->dci[index].id);
}
                                          ...
                                          int index)
{
    ...
    if (uct_rc_fc_has_resources(&iface->super.super, &ep->fc)) {
        ...
        ucs_arbiter_group_schedule(waitq, &ep->dci[index].arb_group);
    }
}
...
static UCS_F_ALWAYS_INLINE int
uct_dc_mlx5_ep_dci_index(uct_dc_mlx5_ep_t *ep, uint8_t dci_index)
{
    if (ep->dci[UCT_DC_MLX5_EP_DCI].id == dci_index) {
        return UCT_DC_MLX5_EP_DCI;
    } else {
        ucs_assert(ep->dci[UCT_DC_MLX5_EP_DCI_RDMA_READ].id == dci_index);
        return UCT_DC_MLX5_EP_DCI_RDMA_READ;
    }
}
...
uct_dc_mlx5_iface_dci_put(uct_dc_mlx5_iface_t *iface, uint8_t dci_index)
{
    ...
    index = uct_dc_mlx5_ep_dci_index(ep, dci_index);
    ...
    if (uct_dc_mlx5_iface_dci_has_outstanding(iface, dci_index)) {
        if (iface->tx.policy == UCT_DC_TX_POLICY_DCS_QUOTA) {
             ...
             */
            if (ep->dci[index].flags & UCT_DC_MLX5_EP_FLAG_TX_WAIT) {
                waitq = uct_dc_mlx5_iface_dci_waitq(iface, pool_index);
                if (!ucs_arbiter_is_empty(waitq)) {
                    return;
                }
                ep->dci[index].flags &= ~UCT_DC_MLX5_EP_FLAG_TX_WAIT;
            }
        }
        ucs_arbiter_group_schedule(uct_dc_mlx5_iface_tx_waitq(iface),
                                   &ep->dci[index].arb_group);
    ...
    ucs_assert(ep->dci[index].id != UCT_DC_MLX5_EP_NO_DCI);
    ep->dci[index].id     = UCT_DC_MLX5_EP_NO_DCI;
    ep->dci[index].flags &= ~UCT_DC_MLX5_EP_FLAG_TX_WAIT;
     ...
     */
    ucs_arbiter_group_desched(uct_dc_mlx5_iface_tx_waitq(iface), &ep->dci[index].arb_group);
    uct_dc_mlx5_iface_schedule_dci_alloc(iface, ep, index);
}

static inline void uct_dc_mlx5_iface_dci_alloc(uct_dc_mlx5_iface_t *iface,
                                               uct_dc_mlx5_ep_t *ep,
                                               int index)
{
    ...
    ucs_assert(!uct_dc_mlx5_iface_is_dci_shared(iface));
    ucs_assert(pool->release_stack_top < pool->stack_top);
    ep->dci[index].id = pool->stack[pool->stack_top];
    ucs_assert(ep->dci[index].id >= (iface->tx.ndci * pool_index));
    ucs_assert(ep->dci[index].id < (iface->tx.ndci * (pool_index + 1)));
    ucs_assert(uct_dc_mlx5_ep_from_dci(iface, ep->dci[index].id) == NULL);
    iface->tx.dcis[ep->dci[index].id].ep = ep;
    pool->stack_top++;
    if (ep->dci[index].flags & UCT_DC_MLX5_EP_FLAG_INVALIDATED) {
        (void)uct_dc_mlx5_ep_qp_to_err(ep, index);
    }

    ucs_assertv(pool->stack_top > 0, "dci pool overflow, stack_top=%d",
                (int)pool->stack_top);
    ucs_trace_data("iface %p: allocate dci %d for ep %p", iface, ep->dci[index].id, ep);
}
...
uct_dc_mlx5_iface_dci_detach(uct_dc_mlx5_iface_t *iface, uct_dc_mlx5_ep_t *ep,
                             int index)
{
    uint8_t dci_index = ep->dci[index].id;
                          ...
                          int index)
{
    ...
    if (uct_dc_mlx5_iface_is_dci_shared(iface)) {
        ...
        if (uct_dc_mlx5_iface_dci_has_tx_resources(iface, dci_index)) {
            return UCS_OK;
        } else {
            UCS_STATS_UPDATE_COUNTER(iface->tx.dcis[dci_index].txqp.stats,
    ...
    if (uct_dc_mlx5_iface_dci_can_alloc(iface, pool_index)) {
        ...
        uct_dc_mlx5_iface_dci_alloc(iface, ep, index);
        return UCS_OK;
    }
...
static inline struct mlx5_grh_av *uct_dc_mlx5_ep_get_grh(uct_dc_mlx5_ep_t *ep,
                                                         int index)
{
   return (ep->dci[index].flags & UCT_DC_MLX5_EP_FLAG_GRH) ?
          &(ucs_derived_of(ep, uct_dc_mlx5_grh_ep_t)->grh_av) : NULL;
}


#define UCT_DC_MLX5_CHECK_DCI_RES(_iface, _ep, _index) \
    { \
        ucs_status_t _status = uct_dc_mlx5_iface_dci_get(_iface, _ep, _index); \
...
#define UCT_DC_CHECK_RES_PTR(_iface, _ep, _index) \
    { \
        { \
            ucs_status_t status = uct_dc_mlx5_iface_dci_get(_iface, _ep, _index); \
 ...
 */
#define UCT_DC_MLX5_CHECK_RES(_iface, _ep, _index) \
    { \
        UCT_DC_MLX5_CHECK_DCI_RES(_iface, _ep, _index) \
        UCT_RC_CHECK_NUM_RDMA_READ_RET(&(_iface)->super.super, \
                                       UCS_ERR_NO_RESOURCE) \
    }
...
#define UCT_DC_CHECK_RES_AND_FC(_iface, _ep, _index) \
    { \
        UCT_DC_MLX5_CHECK_RES(_iface, _ep, _index) \
        ...
        if (!uct_dc_mlx5_iface_is_dci_shared(_iface)) { \
            uct_rc_iface_check_pending(&(_iface)->super.super, \
                                       &(_ep)->dci[_index].arb_group); \
        } \
    }

static UCS_F_ALWAYS_INLINE int
uct_dc_mlx5_iface_dci_id(uct_dc_mlx5_ep_t *ep, int dci_index)
{
    if (ep->dci[UCT_DC_MLX5_EP_DCI].id == dci_index) {
        return UCT_DC_MLX5_EP_DCI;
    }
